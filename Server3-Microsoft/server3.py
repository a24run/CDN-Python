...
#download speeds
low=20
low_resize_width=0.3
medium=40
medium_resize_width=0.5
s=socket.socket();
port = 21611
...
def CreateServerObjects():
    ...
    presentServer=Servers('52.168.141.30',port,"Azure");               #Present Server
        ...
        for each in list_of_servers:
            if(each.port!=port):
                ...
                try:
                    ...
                    heartBeat_socket.send(json.dumps({"ServerCommunication":"heartBeat","heartBeat":"alive","host":"52.168.141.30","port":str(port)}));        #Present Server
