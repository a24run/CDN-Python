...
def CreateServerObjects():
    ...
    presentServer=ServerPrices(host,port);
...
while True:
    ...
    if(data.find("ClientUpload")!=-1):
        ...
        for x in range(0,len(postRequestInput['changes'])):
            print(postRequestInput['changes'][x]['host'])
            for server in list_of_servers_with_price:
                if(server.host==postRequestInput['changes'][x]['host']  and server.port == postRequestInput['changes'][x]['port']):
